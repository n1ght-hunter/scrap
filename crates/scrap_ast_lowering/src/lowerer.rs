...
pub struct EnumInfo<'db> {
    ...
    pub variants: Vec<(Symbol, usize, VariantInfo<'db>)>,
}
...
pub struct ExprLowerer<'db> {
    ...
    pub(crate) symbol_table: HashMap<Symbol, ir::LocalId>,
    /// Struct field name → index mapping for field access resolution.
    /// Key: struct name, Value: map of field_name Symbol → field_index
    pub(crate) struct_fields: HashMap<String, HashMap<Symbol, usize>>,
    /// Enum name → variant info mapping.
    pub(crate) enum_info: HashMap<String, EnumInfo<'db>>,
    /// Names of native Rust interop types in scope. An ADT whose name is here
    /// lowers to the memory-backed [`ir::Ty::Rust`] instead of [`ir::Ty::Adt`].
    pub(crate) rust_type_names: HashSet<String>,
    /// Extra functions generated by `spawn { block }` expressions.
...
impl<'db> ExprLowerer<'db> {
    ...
    pub fn allocate_named_local(&mut self, name: Symbol, ty: ir::Ty<'db>) -> ir::LocalId {
        let id = ir::LocalId(self.local_decls.len());
    ...
    pub fn insert_binding(&mut self, name: Symbol, local: ir::LocalId) {
        self.symbol_table.insert(name, local);
    }

    /// Look up a variable binding in the symbol table
    pub fn lookup_binding(&self, name: Symbol) -> Option<ir::LocalId> {

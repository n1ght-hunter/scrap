...
pub fn lower_module<'db>(
    ...
    rust_types: &[crate::RustTypeLowerInfo],
    ...
    // Rust interop structs aren't in this file's AST (they were synthesized into
    // the type-checked `Can`), so register their field maps here from metadata.
    for rt in rust_types {
        let field_map = rt
            .fields
            .iter()
            .enumerate()
            .map(|(idx, name)| (Symbol::new(name), idx))
            .collect();
        struct_field_maps.insert(rt.name.clone(), field_map);
    }
...
fn lower_body_with_subst<'db>(
    ...
    lowerer.rust_type_names = rust_type_names.clone();
        ...
        match &stmt.kind {
            StmtKind::Let(local) => {
                ...
                if let PatKind::Ident(_, ident, _) = &local.pat.kind {
                    // Prefer the type table (it routes Rust-interop types to
                    // `ir::Ty::Rust`); fall back to the explicit annotation, then
                    // to the table's Bool default for tests without a table.
                    let ty = if let Some(resolved) = lowerer.lookup_local_type(local.id) {
